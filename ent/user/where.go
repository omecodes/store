...
func Validated(v bool) predicate.User {
	return predicate.User(func(s *sql.Selector) {
		s.Where(sql.EQ(s.C(FieldValidated), v))
	})
}
...
func ValidatedNEQ(v bool) predicate.User {
	return predicate.User(func(s *sql.Selector) {
		s.Where(sql.NEQ(s.C(FieldValidated), v))
	})
}
...
func HasAccesses() predicate.User {
	return predicate.User(func(s *sql.Selector) {
		step := sqlgraph.NewStep(
			sqlgraph.From(Table, FieldID),
			sqlgraph.To(AccessesTable, FieldID),
			sqlgraph.Edge(sqlgraph.O2M, false, AccessesTable, AccessesColumn),
		)
		sqlgraph.HasNeighbors(s, step)
	})
}
