...
func (uq *UserQuery) QueryAccesses() *AccessQuery {
	query := &AccessQuery{config: uq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	...
	if query := uq.withAccesses; query != nil {
		...
		query.Where(predicate.Access(func(s *sql.Selector) {
			s.Where(sql.InValues(user.AccessesColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			fk := n.user_accesses
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_accesses" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_accesses" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.Accesses = append(node.Edges.Accesses, n)
		}
	}
