...
func init() {
	ServiceCMD.AddCommand(frontServiceCMD, aclServiceCMD, accessServiceCMD, filesServiceCMD, objectsServiceCMD)
}
...
func init() {
	...
	flags.IntVar(&registryPort, "reg-port", 9090, "Registry server gRPC port")
...
func parseServiceFlags(flags *pflag.FlagSet, withDB bool) error {
	...
	flags.StringVar(&ip, "ip", "", "Bind ip")
	if withDB {
		flags.StringVar(&databaseUri, "db", "", "MySQL database uri")
	}
...
var accessServiceCMD = &cobra.Command{
	Use:   "access",
	Short: "Runs access checker service",
	Run:   func(cmd *cobra.Command, args []string) {},
}
